...
import { type AnchorCustomer, type AnchorCustomerResult, type Customer } from '@/src/types';
...
export function normalizeAnchorCustomer(item: AnchorCustomer): Customer {
  ...
  return {
    company: item.organization_name || item.company || item.company_name || '',
    founder: founderText || item.representative_name || item.founder || item.founder_name || '',
...
export function useAnchorCustomersQuery<TData = Customer[]>(
  options?: Omit<UseQueryOptions<Customer[], Error, TData>, 'queryKey' | 'queryFn'>,
) {
  return useQuery({
...
export function useAnchorCustomers() {
  ...
  const activeCustomers = query.data && query.data.length > 0 ? query.data : [];
