...
import { type ApiServiceItem, type ServiceIdentitySection, type ServiceItem } from '@/src/types';
...
export function normalizeService(raw: ApiServiceItem, index: number): ServiceItem {
  ...
  if (raw.identities && typeof raw.identities === 'object') {
    ...
    if (identityEntries.length > 0) {
      primaryIdentity = {
        ...
        points: Array.isArray(identityEntries[0][1]) ? identityEntries[0][1] : [],
      };
    }
    if (identityEntries.length > 1) {
      secondaryIdentity = {
        title: identityEntries[1][0],
        points: Array.isArray(identityEntries[1][1]) ? identityEntries[1][1] : [],
      };
    }
  }
  ...
  if (!cta || !ctaUrl) {
    ...
    } else if (catLower.includes('incentiwise') || catLower.includes('incentive')) {
...
export function useServicesQuery<TData = ServiceItem[]>(
  options?: Omit<UseQueryOptions<ApiServiceItem[], Error, TData>, 'queryKey' | 'queryFn'>,
) {
  return useQuery({
    ...
    select: (data) => {
      ...
      return sorted.map((item, idx) => normalizeService(item, idx)) as unknown as TData;
    },
    ...options,
  });
}
