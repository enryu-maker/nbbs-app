...
import { fetchIndustries, fetchIndustryById } from '@/src/apis';
...
export function useIndustriesQuery<TData = Industry[]>(
  options?: Omit<UseQueryOptions<Industry[], Error, TData>, 'queryKey' | 'queryFn'>,
) {
  return useQuery({
...
export function useIndustryNamesQuery() {
  return useIndustriesQuery<string[]>({
    select: (data) =>
      data.map((item) => item.name?.trim()).filter((name): name is string => Boolean(name)),
  });
}
...
export function useIndustryQuery(
  id: number,
  options?: Omit<UseQueryOptions<Industry, Error, Industry>, 'queryKey' | 'queryFn'>,
) {
  return useQuery({
