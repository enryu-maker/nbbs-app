...
function createScribble(index: number): string {
  const cx = round(178 + Math.sin(index * 2.37) * 5);
  const cy = round(175 + Math.cos(index * 1.83) * 5);

  const rx = round(114 + Math.sin(index * 1.71) * 26 + Math.cos(index * 0.73) * 18);
  const ry = round(84 + Math.cos(index * 1.29) * 22 + Math.sin(index * 0.91) * 16);

  const rotation = round(-20 + Math.sin(index * 1.41) * 28 + index * 4.2);
...
function createInnerLoop(index: number) {
  const cx = round(180 + Math.sin(index * 2.17) * 7);
  const cy = round(175 + Math.cos(index * 1.73) * 10);

  const rx = round(64 + Math.sin(index * 1.19) * 30);
  const ry = round(46 + Math.cos(index * 1.43) * 20);
  ...
  return (
    ...
    >
      ...
      <div className="absolute top-2 left-[7%] sm:left-[9%] z-10 flex items-center gap-1 sm:gap-1.5 text-[#172039]">
        <span className="font-display italic font-medium text-xs sm:text-sm tracking-wide">
          Chaos
        </span>
        <CornerRightDown className="size-3.5 sm:size-4 text-[#172039]/70" strokeWidth={2} />
      </div>

      {/* CLARITY LABEL */}
      <div className="absolute top-2 right-[6%] sm:right-[8%] z-10 flex items-center gap-1 sm:gap-1.5 text-[#c9a86a]">
        <CornerUpRight className="size-3.5 sm:size-4 text-[#c9a86a]" strokeWidth={2} />
        <span className="font-display italic font-medium text-xs sm:text-sm tracking-wide">
          Clarity
        </span>
      </div>
      ...
      >
        ...
        <defs>
          <clipPath id="hero-circuit-clip">
            <rect x="145" y="20" width="610" height="270" />
          </clipPath>
          <linearGradient
