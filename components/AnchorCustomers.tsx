...
export default function AnchorCustomers() {
      ...
      const timeline = gsap.timeline({
        scrollTrigger: {
          ...
          onUpdate: (self) => {
            ...
            setCurrentSlide((previousSlide) => (previousSlide === slide ? previousSlide : slide));
          },
        },
      });
  ...
  const moveToSlide = (slide: number) => {
    ...
    const progress = customers.length > 1 ? targetSlide / (customers.length - 1) : 0;
  ...
  return (
    <section ref={sectionRef} className="relative isolate z-10 bg-paper py-6 md:py-8">
      {/* HEADER */}
      <div className="w-full px-5 sm:px-8 md:px-10 lg:px-12">
        ...
        <div className="mt-6 flex flex-col gap-6 md:flex-row md:items-end md:justify-between">
          <h2
            className="font-medium max-w-3xl text-3xl font-black leading-[0.92] tracking-[-0.045em] text-navy sm:text-4xl md:text-5xl"
            style={{
        ...
        <div className="overflow-hidden">
          <div ref={trackRef} className="flex">
            {customers.map((customer, index) => (
              <div key={`${customer.company}-${index}`} className="w-full shrink-0 px-1">
                <article
                  className="
                    group
                    relative
                ...
                >
                  ...
                  <div className="grid md:grid-cols-[1.08fr_0.92fr]">
                    ...
                    <div className="flex flex-col px-7 pb-8 pt-8 sm:px-9 sm:pt-9 md:px-11 md:pb-10 md:pt-10">
                      {/* COMPANY */}
                      <h3
                        className="mt-7 max-w-150 font-medium text-3xl font-black leading-[0.98] tracking-[-0.04em] text-navy sm:text-4xl md:text-[2.8rem]"
                        style={{
                          fontFamily: 'Bodoni Moda, serif',
                        }}
                      >
                          ...
                          <div className="grid grid-cols-3">
                            {customer.results.map((result, resultIndex) => (
                              <div
                                ...
                                className={`
                                    ${resultIndex > 0 ? 'border-l border-navy/10 pl-4 sm:pl-5' : ''}
                    ...
                    <div className="flex items-center justify-center bg-[#f4f0e8] px-6 h-[500px] sm:px-9 md:px-10">
                      ...
                      >
                        ...
                        {/* QUOTE MARK */}
                        <div className="font-serif text-6xl leading-none text-[#e9c176]">“</div>
              ...
              {customers.map((_, index) => (
                <button
                  ...
                  className={`
                    ...
                    ${currentSlide === index ? 'w-10 bg-gold' : 'w-2 bg-navy/15 hover:bg-navy/30'}
                  `}
                />
              ))}
            </div>
